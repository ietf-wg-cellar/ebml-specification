...
The `@id` value does not need to be unique within an EBML Schema, but the value of the `@path` of that Element MUST be unique within an EBML Schema. In other words, the Element ID of any Element found within an EBML Document MUST only match a single `@path` of its corresponding EBML Schema, but a separate instance of that Element ID value defined by the EBML Schema MAY  occur within a different `@path`.
