...
| default        | No       | If an Element is mandatory but not written within its Parent EBML Element or stored as an Empty Element, then the EBML Reader of the EBML Document MUST semantically insert this specified default value for the Element. EBML Elements that are Master Elements MUST NOT declare a default value. |
