...
The `maxOccurs` attribute is OPTIONAL. If the `maxOccurs` attribute is not present then no further restrictions are imposed on the number of occurences of this `EBML Element` besides the restriction imposed by `minOccurs`. This is similar to `unbounded` in the XML world.
